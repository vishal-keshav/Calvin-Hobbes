...
//			_StartTarget = GameObject.Find("StartPointStub");
	...
	public static bool IsLocalAnimating(OCAction action, OCActionArgs args)
	{
		...
		{
			isAnimating |= afx.Target.GetComponent<Animation>().IsPlaying(afx.StateName);
		}
	...
	{
		return !args.Source.GetComponent<Animation>().isPlaying;
	}
	...
	{
		return !args.Source.GetComponent<Animation>().isPlaying || args.Source.GetComponent<Animation>().IsPlaying("idle");
	}
...
//				}
	...
	private ActionStatus StartAction()
	{
		...
		if(_blockOnRunning || !_Source.GetComponent<Animation>().isPlaying)
		{
